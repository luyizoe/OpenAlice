...
> 来自对陈成 AI 编程 workflow（2026-05-31）的方法论迁移讨论。
> 原文：https://x.com/chenchengpro/status/2061114453345943921

陈成的核心洞察：**loop 越多，任务自动产出越多，系统越快**——不需要人主动想"下一步做什么"，系统会自己生长。

### 软件工程 vs 投研的迁移差异

| 维度 | 软件工程（陈成）| 投研（本系统）|
|---|---|---|
| 任务最小单元 | GitHub Issue | 研究任务（待核实的信号/判断）|
| 任务执行方式 | AI 立即写代码 | 取决于外部信息是否到位 |
| 反馈循环速度 | 分钟级（跑测试）| 周/月级（等财报/渠道数据）|
| 任务可执行性 | 任意时刻可执行 | 多数需等待特定文档或事件 |

**核心差异**：投研的验证任务天然依赖外部事件（下次电话会、下季度财报、渠道数据更新），这决定了我们的 Loop 不能是"生成即执行"，而必须是**"生成后排队，事件到来时触发执行"**。

### 三类研究任务的分级

```
类型 A：立即可执行（有实时数据 API）
  例："查碳酸锂最新期货价格"
  → 调 API → 立即执行 → 结果写回 coverage 文件

类型 B：排队等文档（依赖特定文档到来）
  例："验证宁德 Q3 采购计划是否放缓"
  → 写入任务队列 → 下次宁德相关文档路由进来时，
     路由系统检查任务队列 → 自动触发该任务的执行

类型 C：需要人主动行动（线下核实）
  例："联系渠道商核实茅台中秋动销"
  → 系统无法执行 → 推送提醒到 Inbox，人去做
```

### 任务排队 + 事件触发执行的架构

```
信号生成阶段（Coverage Agent 完成后）：
  propagation_to: ["battery_cell"] 字段
            ↓
  自动写入任务队列：
  {
    task: "验证中游电芯厂是否感受到成本改善",
    type: "B",  // 等文档类型
    trigger_sectors: ["battery_cell"],
    trigger_keywords: ["宁德", "亿纬", "电芯", "成本"],
    created_by: "lithium_upstream_signal",
    deadline: "2026-07-01",  // 超时变成 Gap Report 条目
    status: "waiting"
  }
            ↓
文档路由阶段（每天）：
  新文档到来 → 路由到对应板块 → 同时检查任务队列
  "是否有等待这类文档的任务？"
            ↓
  有 → 任务状态变为 active → Coverage Agent 处理文档时
       额外执行该任务的验证逻辑
```
...
```
Loop 1：Dream Cycle Gap Report → Inbox 提醒（✅ 已设计，但只到提醒层，未到任务层）
Loop 2：信号 propagation_to → 跨板块传导（✅ 已设计，由 Synthesizer 处理）
Loop 3：竞品/产业主动扫描 → 新任务（❌ 完全缺失）
```

### 需要新增的 Loop

**决策验证 Loop（Phase 3 加入）**
- 建仓时记录决策日志（买入逻辑 + 证伪条件 + 止损位 + verify_by 日期）
- verify_by 日期到达时，Dream Cycle 自动生成"验证此买入逻辑"任务
- 任务状态：等待相关文档 → 自动执行验证 → 写入决策复盘 Timeline

**信号传导验证 Loop（Phase 3 加入）**
- Coverage Agent 写入 propagation_to 信号后，自动生成下游验证任务
- 任务进入等待队列，下次下游板块有文档进来时触发执行
...
本系统：
  定义阶段（人做）：
    - coverage_map.yaml（覆盖范围）
    - AGENTS.md（判断标准——相当于 80% 时间的 design）
    - structure.yaml（产业链结构）
    - 决策日志的买入逻辑和证伪条件

  验收阶段（人做）：
    - 审核每日 delta 报告（今天的更新方向是否正确）
    - 审核 proposals.md（AI 提案的新产业链关系）
    - 审核 Gap Report（哪些地方需要主动补充信息）
    - 审核决策复盘（AI 对判断的验证结论是否准确）
```

**验收标准建议**：每日 delta 审核控制在 10-15 分钟；每周 proposals.md 和 Gap Report 集中审核一次。


---

### 陈成 AI 编程 workflow（2026-06-01 调研）

**来源**：https://x.com/chenchengpro/status/2061114453345943921

**核心方法**：通过 `/one-shot`（全自动化需求流程）+ 并发隔离目录 + Loop（自动生成任务）+ 人只做定义和验收，实现日产出有效代码 1-3 万行。

**迁移到本系统的部分**：
1. **One-shot → Coverage Agent**：把完整研究工作流封装为一次触发、全程不需要人介入的原子任务
2. **隔离 → Workspace 目录隔离**：每个板块独立 Workspace，互不干扰，出问题单独调试
3. **人只做定义 + 验收**：AGENTS.md / structure.yaml / coverage_map.yaml 是"设计阶段"，看 delta 报告是"验收阶段"，中间过程全交给 AI
4. **Loop → 任务排队 + 事件触发**（新增到系统设计，见第十三节）

**迁移不了的部分**：
- 软件任务"任意时刻可执行"；投研任务"依赖外部事件到来"，反馈循环慢 100 倍
- 软件验收有客观标准（测试通过）；投研验收需要主观判断
- 软件 Loop 产出任务可立即执行；投研 Loop 产出任务多数需要排队等文档
